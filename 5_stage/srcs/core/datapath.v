module datapath(
    ...
    // =========================================================================
    //  Hazard Detection Unit (ID stage — looks at EX for load-use)
    // =========================================================================
    ...
    // MEM stage forwarding value (handles ALU results, pc+4, imm; NOT loads)
    assign mem_forward_data = (mem_regwrite_ctrl == 2'b10) ? mem_pc_plus_4 :
                              (mem_regwrite_ctrl == 2'b11) ? mem_imm :
                              mem_alu_out;  // 2'b00 (ALU) — load (01) is stalled, never forwarded from MEM

    // --- Forwarding muxes ---
    // Select most recent value of rs1
    assign ex_forward_rs1 = (forward_a_sel == 2'b10) ? mem_forward_data :
                            (forward_a_sel == 2'b01) ? wb_write_data :
                            ex_rs1_data;

    // Select most recent value of rs2
    ...
    // A: PC (for AUIPC) or rs1
    // B: imm (for I-type, stores, branches) or rs2
    always @(*) begin
    ...
    // --- Branch target computation (runs PARALLEL to ALU, no critical path impact) ---
    ...
    // --- PC select (drives the PC mux in IF — combinational feedback, no register) ---
    assign pc_sel[1] = ex_jalr_ctrl;
    assign pc_sel[0] = ex_jal_ctrl | (ex_branch_en & branch_cond);

    // --- Pipeline flush: any taken branch/jump kills IF and ID ---
    ...
    EX_MEM ex_mem_reg(
        ...
        // Data
        .ex_alu_out(alu_out),
        .ex_rs2_data(ex_forward_rs2),   // forwarded rs2 (raw, shifted in MEM)
        .ex_pc_plus_4(ex_pc_plus_4),
        .ex_imm(ex_imm),
        .ex_rd(ex_rd),
    ...
    // =========================================================================
    //  MEM — Memory Access (loads, stores, byte-lane shifting)
    // =========================================================================

    // --- Store data byte-lane shifting (SB/SH alignment) ---
    ...
    // --- Load extension (byte/half selection + sign extension) ---
    ...
    // Writeback mux — selects the value to write to the register file
    // (register file write port is connected in the ID section above)
    always @(*) begin
